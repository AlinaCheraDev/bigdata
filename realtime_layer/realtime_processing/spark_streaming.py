...
# Create the missing Primary Key columns (ts and hour)
parsed_df = parsed_df \
...
# WRITE TO KAFKA - convert to JSON
...
query_kafka = kafka_output_df.writeStream \
  ...
  .option("checkpointLocation", "/opt/spark/app/checkpoint_kafka") \
  .start()

# WRITE TO CASSANDRA
query_cassandra = parsed_df.writeStream \
  .format("org.apache.spark.sql.cassandra") \
  .option("keyspace", "energy") \
  .option("table", "events_by_hour") \
  .option("checkpointLocation", "/opt/spark/app/checkpoint_cassandra") \
  .outputMode("append") \
  .start()
